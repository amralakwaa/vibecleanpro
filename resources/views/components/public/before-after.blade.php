...
<div {{ $attributes->class(['grid sm:grid-cols-2 gap-6']) }}>
    <div>
        ...
        <img src="{{ $before->url() }}" alt="{{ $before->alt_text ?? '' }}" loading="lazy" class="w-full rounded-2xl aspect-[4/3] object-cover">
    </div>
    <div>
        <x-public.badge tone="accent" class="mb-2">بعد</x-public.badge>
        <img src="{{ $after->url() }}" alt="{{ $after->alt_text ?? '' }}" loading="lazy" class="w-full rounded-2xl aspect-[4/3] object-cover">
    </div>
</div>
